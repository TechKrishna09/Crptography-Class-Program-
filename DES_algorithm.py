...
#
# Plaintext : Exactly 8 characters
...
def string_to_bits(text):
    """Convert 8-character string into 64 bits."""
    bits = []
...
def des_encrypt_block(block, round_keys):
    ...
    # Convert plaintext string to 64 bits
...
def des_decrypt_block(block, round_keys):
    ...
    # IMPORTANT FIX:
    # Convert encrypted bytes into 64 bits.
    # Previously this was:
    #
    # bits = list(block)
    #
    # which produced only 8 values and caused:
    ...
    # Final Permutation
    decrypted_bits = permute(
        combined,
        FP
    )

    # Convert bits back to bytes
    return bits_to_bytes(decrypted_bits)
...
plaintext = input(
    "Enter plaintext (exactly 8 characters): "
)

key = input(
    "Enter key (exactly 8 characters): "
)
...
if len(plaintext) != 8:
    ...
    print("\nError: Plaintext must be exactly 8 characters.")
    exit()
