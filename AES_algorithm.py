# AES-128 Encryption and Decryption
# No external libraries required

S_BOX = [
    
]

INV_S_BOX = [0] * 256

for i in range(256):
    INV_S_BOX[S_BOX[i]] = i


RCON = [
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
]


def xor_bytes(a, b):
    return [x ^ y for x, y in zip(a, b)]


def sub_bytes(state):
    return [S_BOX[x] for x in state]


def inv_sub_bytes(state):
    return [INV_S_BOX[x] for x in state]


def shift_rows(state):
    result = [0] * 16

    for r in range(4):
        for c in range(4):
            result[4 * c + r] = state[4 * ((c + r) % 4) + r]

    return result


def inv_shift_rows(state):
    result = [0] * 16

    for r in range(4):
        for c in range(4):
            result[4 * c + r] = state[4 * ((c - r) % 4) + r]

    return result


def gmul(a, b):
    result = 0

    for _ in range(8):
        if b & 1:
            result ^= a

        high_bit = a & 0x80
        a = (a << 1) & 0xFF

        if high_bit:
            a ^= 0x1B

        b >>= 1

    return result


def mix_columns(state):
    result = state[:]

    for c in range(4):
        i = 4 * c

        a0 = state[i]
        a1 = state[i + 1]
        a2 = state[i + 2]
        a3 = state[i + 3]

        result[i] = (
            gmul(a0, 2) ^
            gmul(a1, 3) ^
            a2 ^ a3
        )

        result[i + 1] = (
            a0 ^
            gmul(a1, 2) ^
            gmul(a2, 3) ^
            a3
        )

        result[i + 2] = (
            a0 ^
            a1 ^
            gmul(a2, 2) ^
            gmul(a3, 3)
        )

        result[i + 3] = (
            gmul(a0, 3) ^
            a1 ^
            a2 ^
            gmul(a3, 2)
        )

    return result


def inv_mix_columns(state):
    result = state[:]

    for c in range(4):
        i = 4 * c

        a0 = state[i]
        a1 = state[i + 1]
        a2 = state[i + 2]
        a3 = state[i + 3]

        result[i] = (
            gmul(a0, 14) ^
            gmul(a1, 11) ^
            gmul(a2, 13) ^
            gmul(a3, 9)
        )

        result[i + 1] = (
            gmul(a0, 9) ^
            gmul(a1, 14) ^
            gmul(a2, 11) ^
            gmul(a3, 13)
        )

        result[i + 2] = (
            gmul(a0, 13) ^
            gmul(a1, 9) ^
            gmul(a2, 14) ^
            gmul(a3, 11)
        )

        result[i + 3] = (
            gmul(a0, 11) ^
            gmul(a1, 13) ^
            gmul(a2, 9) ^
            gmul(a3, 14)
        )

    return result


def key_expansion(key):
    key = list(key)
    round_keys = [key[:]]

    for round_num in range(1, 11):
        previous = round_keys[-1]
        temp = previous[12:16]

        # RotWord
        temp = temp[1:] + temp[:1]

        # SubWord
        temp = [S_BOX[x] for x in temp]

        # Rcon
        temp[0] ^= RCON[round_num]

        new_key = []

        # First word
        for i in range(4):
            new_key.append(previous[i] ^ temp[i])

        # Remaining words
        for i in range(4, 16):
            new_key.append(previous[i] ^ new_key[i - 4])

        round_keys.append(new_key)

    return round_keys


def add_round_key(state, key):
    return xor_bytes(state, key)


def aes_encrypt(plaintext, key):
    round_keys = key_expansion(key)

    state = list(plaintext)

    # Initial AddRoundKey
    state = add_round_key(state, round_keys[0])

    # Rounds 1-9
    for round_num in range(1, 10):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[round_num])

    # Final round
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[10])

    return bytes(state)


def aes_decrypt(ciphertext, key):
    round_keys = key_expansion(key)

    state = list(ciphertext)

    # Initial AddRoundKey
    state = add_round_key(state, round_keys[10])

    # Rounds 9-1
    for round_num in range(9, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, round_keys[round_num])
        state = inv_mix_columns(state)

    # Final round
    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, round_keys[0])

    return bytes(state)


# ---------------------------------------------------
# MAIN PROGRAM
# ---------------------------------------------------

print("========== AES-128 ==========")

plaintext = input("Enter plaintext (exactly 16 characters): ")
key = input("Enter key (exactly 16 characters): ")

if len(plaintext) != 16:
    print("Error: Plaintext must be exactly 16 characters.")
    exit()

if len(key) != 16:
    print("Error: Key must be exactly 16 characters.")
    exit()

plaintext_bytes = plaintext.encode("utf-8")
key_bytes = key.encode("utf-8")

# Encryption
ciphertext = aes_encrypt(plaintext_bytes, key_bytes)

print("\nOriginal Text :", plaintext)
print("Key           :", key)
print("Encrypted HEX :", ciphertext.hex().upper())

# Decryption
decrypted = aes_decrypt(ciphertext, key_bytes)

print("Decrypted Text:", decrypted.decode("utf-8"))
